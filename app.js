...
const dropMenu = document.querySelector(".drop-nav-item");
const dropMenuItems = document.querySelectorAll(".drop-nav-items");
...
for (let i = 0; i < portfolioItem.length; i++) {
  portfolioItem[i].addEventListener("click", () => {
    ...
    for (let e = 0; e < portImg.length; e++) {
      ...
      if (
        dataItem === portImg[e].getAttribute("data-filtering") ||
        dataItem == "all"
      ) {
...
hamburger.addEventListener("click", () => {
  ...
  dropMenuItems.forEach((item) => {
    item.addEventListener("click", () => {
      home.classList.remove("dropBtn");
      hamburger.classList.remove("clicked");
    });
  });
  home.classList.toggle("dropBtn");
});
...
var typed = new Typed(".animate", {
  strings: ["Teacher", "Developer", "Web Designer"],
  typeSpeed: 150,
  backSpeed: 75,
  loop: true,
});
